...
def CAN_3D(n_frame, nb_filters1, nb_filters2, input_shape, kernel_size=(3, 3, 3), dropout_rate1=0.25, dropout_rate2=0.5,
    ...
    diff_input = Input(shape=input_shape)
    rawf_input = Input(shape=input_shape)
    d1 = conv3D_T(diff_input, nb_filters1, kernel_size, padding='same', activation='tanh')
    # d1 = CDC_ST(nb_filters1, kernel_size, padding='same', activation='tanh')(diff_input)
    # d1 = CDC_ST(nb_filters1, kernel_size, padding='same', activation='tanh')(diff_input)
    d2 = conv3D_T(d1, nb_filters1, kernel_size,padding='same', activation='tanh')

    # Appearance Branch
    # r1 = CDC_ST(nb_filters1, kernel_size, padding='same', activation='tanh')(rawf_input)
    r1 = conv3D_T(rawf_input, nb_filters1, kernel_size, padding='same', activation='tanh')
    r2 = conv3D_T(r1, nb_filters1, kernel_size,padding='same', activation='tanh')
    g1 = conv3D_T(r2, 1, (1, 1, 1), padding='same', activation='sigmoid')
    g1 = Attention_mask()(g1)
    gated1 = multiply([d2, g1])
    d3 = AveragePooling3D(pool_size)(gated1)

    d4 = Dropout(dropout_rate1)(d3)
    # d5 = CDC_ST(nb_filters2, kernel_size, padding='same', activation='tanh')(d4)
    # d6 = CDC_ST(nb_filters2, kernel_size, activation='tanh')(d5)
    d5 = conv3D_T(d4, nb_filters2, kernel_size, padding='same', activation='tanh')
    d6 = conv3D_T(d5, nb_filters2, kernel_size,padding='same', activation='tanh')

    r3 = AveragePooling3D(pool_size)(r2)
    r4 = Dropout(dropout_rate1)(r3)
    # r5 = CDC_ST(nb_filters2, kernel_size, padding='same', activation='tanh')(r4)
    # r6 = CDC_ST(nb_filters2, kernel_size, activation='tanh')(r5)
    # g2 = CDC_ST(1, (1, 1, 1), padding='same', activation='sigmoid')(r6)
    r5 = conv3D_T(r4, nb_filters2, kernel_size, padding='same', activation='tanh')
    r6 = conv3D_T(r5, nb_filters2, kernel_size,padding='same', activation='tanh')
    g2 = conv3D_T(r6, 1, (1, 1, 1), padding='same', activation='sigmoid')
    g2 = Attention_mask()(g2)
    gated2 = multiply([d6, g2])
